...
public class UserUtil {
    ...
    private static void createUser(int count) throws Exception {
        List<Users> users = new ArrayList<Users>(count);
        //生成用户
        for (int i = 0; i < count; i++) {
            ...
            user.setSalt("1a2b3c");
            user.setPassword(Md5Util.toDbEncode("123456", user.getSalt()));
            users.add(user);
        }
...
//        登录，生成token
        ...
        for (int i = 0; i < users.size(); i++) {
            ...
            String params = "mobile=" + user.getId() + "&password=" + Md5Util.firstEncode("123456");
